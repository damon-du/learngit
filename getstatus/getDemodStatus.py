...
def get_channel_status(sport, pstr=''):
    ...
    if 'CF' in pstr:
        ...
        # 遥控频率
        if sport == 952:
            dict_xdstatus['TCUF'] = list_convert[0].split('_')[1].replace(
                '\r\n', '')
        # 跟踪频率
        if sport == 950:
            antf = 11000 - \
...
def get_bytes_time():
    ...
    btime = integral_Seconds.to_bytes(length=4,
                                      byteorder='little',
                                      signed=False)
    stime = btime.hex()
    return btime
...
def pack_auto_recieve():
    ...
    sniff(iface='Intel(R) I350 Gigabit Network Connection',
          filter=
          'ip src 172.30.61.26 or ip src 172.30.61.32 or ip src 172.30.61.21',
          prn=pack_callback,
          count=0,
          store=0)
...
def get_demode_status():
    ...
    list_module_id = [
        '00003010', '00003011', '00003070', '00003071', '00003072', '00003073',
        '00003001'
    ]
    global dict_alarm
            ...
            for mid in list_module_id:
                ...
                if (frame_resp_mid == b'\x00\x00\x30\x10'):
                    ...
                    dict_dmstatus['DT1D'] = str(
                        round(
                            bytes_to_float(list_bytes_recieve[44]) / 1000, 2))
                # 解调消息块2
